...
public class Switch {
    ...
    public static void set(String deviceID, String accessID, String secret, boolean state) {
        ...
        if (error != null) {
            // error
            // only send every 5 mins to reduce noise
            if (LocalTime.now().getMinute() % 10 == 0 && LocalTime.now().getSecond()<50) { // <50 ensures a script called every min doesn't accidentally pass the %10 test twice in the same minute
